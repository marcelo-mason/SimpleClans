...
public class HomeCommand extends GenericPlayerCommand
{
    ...
    public void execute(Player player, String label, String[] args)
    {
        ...
        if (args.length == 2) {
            if (plugin.getPermissionsManager().has(player, "simpleclans.mod.home")) {
                Clan clan = plugin.getClanManager().getClan(args[1]);
                if (clan != null) {
                    Location home = clan.getHomeLocation();
                    if (args[0].equalsIgnoreCase(plugin.getLang("home.command.set"))) {
                        ...
                        if (home != null) {
                            ...
                            player.sendMessage(ChatColor.AQUA + MessageFormat.format(plugin.getLang("now.at.homebase"), clan.getName()));
                        } else {
